"""
HarmActionsBench Benchmark Framework
================================

HarmActionsBench is a dataset-driven benchmark that integrates Agent Action Guard's
HarmActionsBench evaluation suite into DeepTeam. It assesses whether an AI agent
...
# Acknowledgement:
# This benchmark is based on Agent Action Guard's HarmActionsBench,
...
from deepteam.frameworks.harmactionsbench.types import HarmActionsBenchType
...
class HarmActionsBench(AISafetyFramework):
    name = "HarmActionsBench"
    description = (
    ...
    def _to_test_case(self, action: Dict[str, Any]) -> RTTestCase:
        ...
        return RTTestCase(
            ...
            vulnerability_type=HarmActionsBenchType.HARMACTIONSBENCH,
            metadata=metadata,
        )
    ...
    def load_dataset(self):
        ...
        if self.offset >= len(harmful_actions):
            raise ValueError(
                "offset is beyond the available harmful actions in HarmActionsBench dataset"
            )
    ...
    async def _evaluate_test_case(
        ...
        try:
            ...
            if ignore_errors:
                test_case.error = (
                    "Error evaluating HarmActionsBench tool-call behavior"
                )
                test_case.score = 0.0
