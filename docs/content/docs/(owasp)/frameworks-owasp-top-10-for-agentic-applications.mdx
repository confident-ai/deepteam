...
:::warning
Memory poisoning can have long-lasting effects since agents rely on historical context for decision-making. Implement memory validation, integrity checks, and periodic memory audits. An open-source implementation of these controls is [OWASP Agent Memory Guard](https://github.com/OWASP/www-project-agent-memory-guard), which also publishes a labelled memory-poisoning corpus useful as an attack seed set.
:::
