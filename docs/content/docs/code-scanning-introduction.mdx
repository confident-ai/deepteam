...
There are **EIGHT** optional command line flags:
...
- [Optional] `-m`, `--model`: evaluation model, e.g. `gpt-4o`. Overrides the config.
...
There are **SEVEN** optional parameters when creating a code scan configuration:
...
- [Optional] `model`: default evaluation model. Defaulted to `None`.
